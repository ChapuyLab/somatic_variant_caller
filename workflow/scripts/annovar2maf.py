...
def read_annovar_file(file_path, meta, protocol):
    with open(file_path, 'r') as annovar_file:
        ...
        for line in annovar_file:
            ...
            yield('\t'.join([str(x) for x in maf_line]))
...
if __name__ == "__main__":
    ...
    meta = '\t'.join([str(x) for x in [snakemake.params.get("tsb"), snakemake.params.get("build"), "NA"]])

    with open(snakemake.output['maf'], 'w') as handle:
        for output_line in read_annovar_file(snakemake.input["annotated"], meta, snakemake.params.get("protocol")):
