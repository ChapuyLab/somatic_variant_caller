...
class RealignmentFilter:
    ...
    def collect_blat_queries(self, stepper='samtools'):
    ...
    def collect_mate_blat_queries(self, stepper='samtools'):
    ...
    def write_new_mafs_all_reject(self):
        """Write three new mafs: new_maf contains both passed and rejected variants; debug_maf contains rejected variants; pass_maf contains passed variants."""
        with open(self._out_name + ".blat.all.maf", "w") as new_maf, open(self._out_name + ".blat.passed.maf",
                                                                          "w") as pass_maf, open(
                self._out_name + ".blat.rejected.maf", "w") as debug_maf:
            ...
            def write_maf_record(row):
                ...
                if row['judgement'] == 'REJECT':
                    ...
                    pass_maf.write(new_row)
            self._input_maf.variant_df['judgement']='REJECT'
            self._input_maf.variant_df.apply(write_maf_record, axis=1)

            ## only doing this for making snakemake happy otherwise these file dont make sense and will be cleaned up anyway
            with open(self._blat_read_query_filename, 'w') as handle:
                pass
            with open(self._blat_read_query_output_psl_filename, 'w') as handle:
                pass
            with open(self._blat_mate_query_filename, 'w') as handle:
                pass
            with open(self._blat_mate_query_output_psl_filename, 'w') as handle:
                pass
