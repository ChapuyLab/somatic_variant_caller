...
class input:
    ...
    def read_call_stats_file(self):
        fields = ['contig', 'position', 'ref_allele', 'alt_allele', 'tumor_name', 'normal_name', 't_alt_count',
                  't_ref_count'
            , 'n_alt_count', 'n_ref_count', 'failure_reasons', 'judgement','t_lod']
    ...
    def read_het_file(self):
        if self.tumor_het_file=='NULL' or self.normal_het_file=='NULL':
...
def main():
    ...
    if 'snakemake' in globals():
        args = SimpleNamespace(
            mutation_data_path=snakemake.input.get("mutation_data_path", "NULL"),
            cn_data_path=snakemake.input.get("cn_data_path", "NULL"),
            tumor_het_data_path=snakemake.input.get("tumor_het_data_path", "NULL"),
            normal_het_data_path=snakemake.input.get("normal_het_data_path", "NULL"),
            exac_data_path=snakemake.input.get("exac_data_path", None),
            indel_data_path=snakemake.input.get("indel_data_path", "None"),

            indel_data_type=snakemake.params.get("indel_data_type", "None"),

            output_name=snakemake.params["output_name"],
            output_dir=snakemake.output.get("output_dir", "."),

            mutation_prior=snakemake.params.get("mutation_prior", 0.05),
            aSCNA_threshold=snakemake.params.get("aSCNA_threshold", 0.1),
            TiN_prior=snakemake.params.get("TiN_prior", 0.5),
            use_outlier_removal=snakemake.params.get("use_outlier_removal", True),
            resolution=snakemake.params.get("resolution", 101),
            weighted_classification=snakemake.params.get("weighted_classification", False),
            ascna_probe_number_filter=snakemake.params.get("ascna_probe_number_filter", 200),
            ascna_SNP_number_filter=snakemake.params.get("ascna_SNP_number_filter", 20),
            coverage_threshold=snakemake.params.get("coverage_threshold", 15),
            SSNV_af_threshold=snakemake.params.get("SSNV_af_threshold", 0.2),
            aSCNA_variance_threshold=snakemake.params.get("aSCNA_variance_threshold", 0.025),
            cancer_hot_spots=snakemake.input.get("cancer_hot_spots", "NA"),
            only_ascnas=snakemake.params.get("only_ascnas", False),
            )
    else:
